...
# @LogMethodCalls
class MonteCarloWithDiff:
  def __init__(self, heterodata, edge_index, model, edge, number_of_brother, deepnes_of_node_expansion=8,
    ...
    #### Quando viene passato un edge_index = None, significa che non voglio che venga fatto il subgraph, quindi uso tutto il grafo
    if self.edge_index is not None:
      positions = find_sub_edge_positions(self.data['user', 'rates', 'item'].edge_index,
                                          self.edge_index)  ## TODO Questa funzione funziona, ma è molto lenta, velocizzarla
    ...
    #### Da leveare in produzione
    self.prediction_to_evaluate_index = get_index_starting_from_nodes(
      edge_index=self.data['user', 'rates', 'item'].edge_index, edge=self.edge)
  ...
  def simulation_montecarlo(self, node_id, edge_index):
    ...
    ## Genero un grafo conforme al root_node e procedo alla simulazione
    ...
    for valore in list_of_edges_to_remove:
      if valore in list_of_index:
        list_of_index.remove(valore)
    ...
    win = (1 / diff)
    win = win.item()
    win = win/100000 ## Perchè SQL non accetta valori troppo grandi
    ...
    with open(ROOT_DIR + '/logs/' + str(today) + '.txt', mode='a') as f:
      f.write('current_time: ' + str(datetime.now()) + ';edge_index: ' + str(win_dic['edge_index']).replace("\n",
        ...
        self.deepnes_of_node_expansion).replace("\n", "") + ';win: ' + str(win_dic['win']).replace("\n",
                                                                                                   "") + ';difference_in_prediction: ' + str(
        win_dic['diff']) + ';number_of_brother: ' + str(self.number_of_brother) + '\n')

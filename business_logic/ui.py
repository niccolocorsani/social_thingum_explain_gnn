...
class MainWindow(QMainWindow):
  ...
  def initUI(self):
    ...
    self.run_neo4j_button = QPushButton('Get data from neo4j')
    ...
    # Create and add run simulation button to the vertical layout
    self.run_simulation_button = QPushButton('Run montecarlo tree search')
  ...
  def run_simulation(self):
    ...
    print(len(self.data['user', 'rates', 'movie'].edge_index[0]))
    user = self.edge_index[0][int(prediction_index)] ## Prendo il nodo a partire dall'arco specificato
    self.output_text.append('Inizio recezione grafo con neo4j su arco identificato come: '+ str(self.edge_index[0][int(prediction_index)])+str(self.edge_index[1][int(prediction_index)]))

    ## TODO mettere edge_index in modo tale da tagliarlo per l'ultimo nodo user.....
    self.edge_index, _ = neo4j_queries.get_subgraph_from_neo4j_to_explainability(user_number=str(user.item()), number_of_brothers=int(self.number_of_brothers.text()))
    self.output_text.append('.......' + '\n')
    self.output_text.append('edge_index dimensione iniziale: ' + str(len(self.edge_index[0])) + '\n')
    ...
    monte_carlo = MonteCarlo(heterodata=self.data, edge_index=self.edge_index, deepnes_of_node_expansion=int(deepnes),
                             ...
                             prediction_to_evaluate_index=int(prediction_index), edge=edge, number_of_brother=self.number_of_brothers)

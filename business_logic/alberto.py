...
def remove_duplicate_columns(tensor_indices):
...
def find_sub_edge_positions(edge_index, sub_edge_index):
  ...
  for i in range(sub_edge_index.shape[1]):
    ...
    for j in range(edge_index.shape[1]):
      var1 = sub_edge_index_t[i]
      var2 = edge_index_t[j]
      if (sub_edge_index_t[i] == edge_index_t[j]).all():
        contained = True
        positions.append(j)
    if contained == False:
      print("Error: edge:" + str(sub_edge_index_t[i]) + " is not contained in edge_index")
...
def get_hetero_data():
  ...
  return data, df, list(df.enc_user), list(df.enc_item)


def filtra_lv1(df, source_node_userID, target_node_itemID):
  request = requests.get(
    'https://whoteach-scraper-microservice-agpvmcasea-oc.a.run.app/user/ratingsData', auth=HTTPBasicAuth(
      'dbZjdqRpqCup@neo4j', '30bqlw8c325D1AtK'), headers={'Accept': 'application/json'})
...
def filtra_lv_2(df, source_node_userID, target_node_itemID):
  uri = "neo4j://34.154.222.156:7687"
  user = "neo4j"
  password = "Ontologia235g!"
  myNeo4j = MyNeo4j(uri, user, password)
  ...
  results = myNeo4j.run_query(
    'MATCH (u:User{userId:"https://www.merlot.org/merlot/viewMember.htm?id=248734"})-[r:HAS_RATED]->(a:Article)<-[r1:HAS_RATED]-(u1:User)-[r2:HAS_RATED]->(a1:Article) return u.userId, id(a), u1.userId, id(a1)')
  ...
  for element in results:
    ...
    try:
      ...
      continue
    if df[(df.itemId == element['id(a)']) & (df.userId == element['u.userId'])].empty:
      continue
  ...
  for element in results:
    ...
    if df[(df.itemId == element['id(a)']) & (df.userId == element['u1.userId'])].empty:
      continue
  ...
  for element in results:
    ...
    try:
      ...
      continue
    if df[(df.itemId == element['id(a1)']) & (df.userId == element['u1.userId'])].empty:
      continue
  ...
  return torch.tensor(edge_index_to_test), edge
...
if __name__ == '__main__':
  ...
  data, df, enc_users, enc_items = get_hetero_data()

  data_clone = data.clone()

  final_hetero_data, final_predictions, model = allAlberto(data_clone)

  edge_index, edge = filtra_lv_2(df, source_node, target_node)
  ...
  mytensor = utility.readTensorFromFile(ROOT_DIR + '/all_edges.txt')

  for i in range(mytensor[0].size(0)):
    element = mytensor[0][i].item()

  ## Per ogni elemento del tensore stamapalo, ricorda che è dimensione (2XN)

  # montecarlo = MonteCarlo(heterodata=data, edge_index=edge_index, deepnes_of_node_expansion=int(2),
